...
interpolateQuaternions <- function(data, quatCols, artifact_indices) {
  ...
  n <- nrow(data)
  artifact_indices <- c(1949, artifact_indices)
...
# Function to calculate quaternion difference
quatDiff <- function(q1, q2) {
    # Assuming quaternions are normalized
    dot_product <- sum(q1 * q2)
    return(2 * acos(abs(dot_product))) # Absolute value ensures the shortest path
}
...
filterArtifacts <- function(data, trackerName, velocity_threshold = 10, rotational_velocity_threshold = 3, passes = 1) { # velocity_threshold in m/s , rotational_velocity_threshold in rad/s
  ...
  if (length(posCols) != 3) {
    stop("Error: found more than 3 position columns.")
  }
  ...
  for (col in trackerCols) {
    artifact_indices <- c(artifact_indices, which(tempData[[col]] == 0))
  }
  ...
  for (i in 1:passes) {
    ...
    tempData <- tempData %>%
      mutate(
          diff_x = c(NA, diff(get(posCols[1]))),
          diff_y = c(NA, diff(get(posCols[2]))),
          diff_z = c(NA, diff(get(posCols[3]))),
          diff_time = c(NA, diff(time)),
      )%>%
      mutate(
    ...
    # Calculate quaternion rotational velocity
    rotational_velocity <- rep(NA, nrow(tempData))
    for (j in 2:nrow(tempData)) {
        q1 <- tempData[j - 1, quatCols]
        q2 <- tempData[j, quatCols]
        angle_diff <- quatDiff(q1, q2)
        rotational_velocity[j] <- angle_diff / tempData$diff_time[j]
    }
    ...
    # Mark artifacts
    data$artifact[velocity_artifact_indices] <- trackerName

    # Interpolate
    tempData[velocity_artifact_indices, trackerCols] <- NA
...
#filterArtifacts(trackerData, "LeftFoot") # example usage

filterAllData <- function(data, velocity_threshold = 10, rotational_velocity_threshold = 500, passes = 1) {
  ...
  data <- filterArtifacts(data, "TreadmillLeft", rotational_velocity_threshold, velocity_threshold, passes)
  data <- filterArtifacts(data, "LeftFoot", velocity_threshold, rotational_velocity_threshold, passes)
  #data <- filterArtifacts(data, "RightFoot", velocity_threshold, passes)
...
renderPlot({
  ...
  data <- filterAllData(data, input$maxv, input$maxvRot, 1)
...
plotData2D <- function(data, x_axis, y_axis) {
  ...
  data <- data[trackerData$time >= input$start & trackerData$time <= input$end, ]
...
quaternionToUpVector <- function(quaternion) {
  # Convert quaternion to rotation matrix
  rotMatrix <- quaternionToRotationMatrix(quaternion)
  # Extract 'up' vector (second row of the rotation matrix)
  upVector <- rotMatrix[2, 1:3]
  return(upVector)
}
...
calculateTransformationMatrix <- function(trackerData) {
  ...
  leftUp <- quaternionToUpVector(colMeans(trackerData[1:100, c("TreadmillLeft.rot.w", "TreadmillLeft.rot.x", "TreadmillLeft.rot.y", "TreadmillLeft.rot.z")]))
  rightUp <- quaternionToUpVector(colMeans(trackerData[1:100, c("TreadmillRight.rot.w", "TreadmillRight.rot.x", "TreadmillRight.rot.y", "TreadmillRight.rot.z")]))
  yAxis <- (leftUp + rightUp) / 2
  yAxis <- yAxis / sqrt(sum(yAxis^2))  # Normalize

  # Z Axis: Orthogonal to X and Y
  zAxis <- vectorCrossProduct(xAxis, yAxis)
  zAxis <- zAxis / sqrt(sum(zAxis^2))  # Normalize

  # Create the rotation matrix
  rotationMatrix <- rbind(xAxis, yAxis, zAxis)
...
```{r,echo=FALSE}
inputPanel(
  numericInput("3dplotheight", "Plot Height",
            min = 50, max = Inf, value = 500, step = 50)
)
cols3d <- grep("LeftFoot.pos", colnames(trackerData), value = TRUE)
...
renderRglwidget({
  ...
  # Create your 3D plot
  # Assuming transformedData is available globally
  lines3d(data[[cols3d[1]]], data[[cols3d[2]]], data[[cols3d[3]]], color="black")
  #specialPoints <- transformedData[transformedData$artifact, ]
  #points3d(specialPoints$x, specialPoints$y, specialPoints$z, color="red", size=3)
