...
calculateTransformationMatrix <- function(trackerData) {
  ...
  rotationMatrix = quaternionToRotationMatrix(c(1,0,0,0))
...
### Filtering artefacts
We also need to filter out artefacts from our dataset (points at which the trackers disconnected). We identify these by using a threshold velocity

```{r}
filterArtifacts <- function(data, velocity_threshold = 10, passes = 1) {
  data$artifact <- FALSE  # Initialize the artifact column
  for(i in 1:passes) {
    # Calculate Velocity
    data <- data %>%
      mutate(
        diff_x = c(NA, diff(pos.x)),
        diff_y = c(NA, diff(pos.y)),
        diff_z = c(NA, diff(pos.z)),
    ...
    # Identify artifacts based on velocity threshold
    artifact_indices <- which(data$velocity > velocity_threshold)
    data$artifact[artifact_indices] <- TRUE  # Mark artifacts

    # Filter out artifacts for interpolation
    tempData <- data
    tempData[artifact_indices, c("pos.x", "pos.y", "pos.z")] <- NA

    # Interpolate missing data
    tempData$pos.x <- na.approx(tempData$pos.x)
    tempData$pos.y <- na.approx(tempData$pos.y)
    tempData$pos.z <- na.approx(tempData$pos.z)

    # Replace position data with interpolated data
    data[, c("pos.x", "pos.y", "pos.z")] <- tempData[, c("pos.x", "pos.y", "pos.z")]
  }
...
# Usage example:
#filterArtifacts(transformTrackerData(participants[1], "left", 1),10)
```

### Plot trajectories

First thing to do is check if our transform works. We also want to double check the points we filter out to make sure we are not discarding important data. Easiest way to verify is to plot the trajectories:
...
makeLinePlot = reactive({
  data <- transformTrackerData(input$participant, input$foot, input$TrialNumber, input$start, input$end)
  #data = filterArtifacts(data, input$maxv)
  # Create plot
  return(plotTransformedData(data, input$xplot, input$yplot))
})
...
plotTransformedData <- function(transformedData, x_axis = "time", y_axis = "pos.x") {
  p <- ggplot() +
    geom_line(data = transformedData, aes_string(x = x_axis, y = y_axis), color = "black") +
    geom_point(data = transformedData[transformedData$artifact, ], aes_string(x = x_axis, y = y_axis), color = "red") +
